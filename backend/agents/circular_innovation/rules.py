...
def ask_llm_for_pathways(material_type: str, quantity_kg: float) -> List[PathwayItem]:
    """Helper calling Google Gemini to suggest circular pathways for a material."""
    prompt = (
        f"Suggest 2-3 realistic circular-economy reuse pathways for the material '{material_type}' "
        f"with a quantity of {quantity_kg} kg.\n"
        f"Respond ONLY in this exact JSON format:\n"
        f"[\n"
        f"  {{\"pathway\": \"short pathway name\", \"notes\": \"1-2 sentence explanation\"}},\n"
        f"  {{\"pathway\": \"short pathway name\", \"notes\": \"1-2 sentence explanation\"}}\n"
        f"]"
    )
    
    # Try gemini-1.5-flash first (as requested), fallback to gemini-3.5-flash on deprecation 404
    try:
        model = genai.GenerativeModel("gemini-1.5-flash")
        response = model.generate_content(prompt)
    except Exception as e:
        logger.warning(f"gemini-1.5-flash failed or unavailable. Falling back to gemini-3.5-flash. Error: {e}")
    ...
    pathways = []
    for item in parsed:
        pathways.append(PathwayItem(pathway=item["pathway"], notes=item["notes"]))
    return pathways



def find_innovation_pathways(material_type: str, quantity_kg: float) -> InnovationResult:
    """Entry point finding circular pathways in curated dataset or via Gemini LLM fallback."""
    data_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data"))
    json_path = os.path.join(data_dir, "circular_innovation_cases.json")
    ...
    if os.path.exists(json_path):
        try:
            ...
            logger.warning(f"Failed to parse curated cases JSON: {e}")
            
    mat_key = material_type.lower().strip().replace(" ", "_")
    if mat_key in cases:
        pathways = [PathwayItem(pathway=p["pathway"], notes=p["notes"]) for p in cases[mat_key]]
    ...
    # Not found in curated JSON - Query Gemini LLM
    try:
        pathways = ask_llm_for_pathways(material_type, quantity_kg)
        return InnovationResult(
            material=material_type,
            quantity=quantity_kg,
            pathways=pathways,
        ...
        print(f"WARNING: Gemini API call failed or response malformed. Error: {e}")
        logger.warning(f"Gemini pathway retrieval failed: {e}")
        fallback = [
            PathwayItem(
                pathway="Waste-to-energy conversion",
                notes="Controlled incineration for energy recovery is a documented last-resort option for non-hazardous waste."
            )
        ]
        return InnovationResult(
            material=material_type,
